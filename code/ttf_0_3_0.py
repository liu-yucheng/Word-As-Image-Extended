...
def fix_single_svg(svg_path, all_word=False):
    target_h_letter = 256
    target_canvas_width, target_canvas_height = 300, 300
...
def glyph_to_cubics(face, x=0, y=0):
    ...
    beziers = []
    def pt(p): return np.array([p.x + x, -p.y - y])  # Flipping here since freetype has y-up
...
def font_string_to_beziers(font, txt, size=60, spacing=1.0, merge=True, target_control=None):
    ...
    for line in lines:
        ...
        for c in line:
            ...
            previous = c
        y += (slot.advance.x + kerning.x) * spacing * 1.75
...
def font_string_to_svgs(dest_path, font, txt, size=48, spacing=1.0, target_control=None, subdivision_thresh=None):
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" baseProfile="full" '''
    ...
    for i, (c, beziers) in enumerate(zip(txt_, glyph_beziers)):
        print(f"==== {c} ====")
