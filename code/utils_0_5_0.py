...
import save_svg_0_5_0 as save_svg
from ttf_0_5_0 import font_string_to_svgs, normalize_letter_size
...
def get_letter_ids(letter, word, shape_groups):
    ...
    for group, l in zip(shape_groups, word_):
        if l == letter:
...
def combine_word(word, letter, font, experiment_dir):
    ...
    if (letter_ == word_):
        shapes_word = shapes
        shape_groups_word = shape_groups
    else:
        for j, s in enumerate(letter_ids):
            shapes_word[s] = shapes[j]

    save_svg.save_svg(
        f"{experiment_dir}/{font}_{word_}_{letter_}.svg", canvas_width, canvas_height, shapes_word,
        shape_groups_word)
    ...
    img = img[:, :, :3]
    save_image(img, f"{experiment_dir}/{font}_{word_}_{letter_}.png")
